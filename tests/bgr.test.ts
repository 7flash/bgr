...
describe("bgr Process Manager Fixes", () => {
  test("Bug 1 Fix: Should start a process successfully when default .config.toml is missing", async () => {
    ...
    const db = new Database(dbPath);
    const proc = db
      .query(`SELECT * FROM process WHERE name = ?`)
      .get("app-no-config") as any;
    db.close();
  ...
  test("Bug 2 Fix: Should remember and use custom config path on restart", async () => {
    ...
    await fs.writeFile(
      printEnvScript,
      `console.log(process.env.SETTINGS_MY_VAR);\n`,
    );
    ...
    expect(startResult.stdout).toContain(
      '🚀 Launched process "app-with-config"',
    );
    ...
    expect(restartResult.stdout).toContain(
      `Loaded config from ${customConfigName}`,
    );
    expect(restartResult.stdout).toContain(
      '🔄 Restarted process "app-with-config"',
    );
