...
beforeAll(async () => {
  ...
  expect(existsSync(join(packageRoot, "dashboard", "lib", "runtime.ts"))).toBe(
    true,
  );

  runtimeMod = await import(
    pathToFileURL(join(packageRoot, "dashboard", "lib", "runtime.ts")).href
  );
  versionRoute = await import(
    pathToFileURL(
      join(packageRoot, "dashboard", "app", "api", "version", "route.ts"),
    ).href
  );
  guardEventsRoute = await import(
    pathToFileURL(
      join(packageRoot, "dashboard", "app", "api", "guard-events", "route.ts"),
    ).href
  );
  historyRoute = await import(
    pathToFileURL(
      join(packageRoot, "dashboard", "app", "api", "history", "route.ts"),
    ).href
  );
...
afterAll(() => {
  try {
    if (runtimeMod?.db?.history) {
      const rows = runtimeMod.db.history
        .select()
        .where({ process_name: "packaged-history-proc" })
...
describe("installed package dashboard routes", () => {
  ...
  test("packaged history route can write and read via installed DB runtime", async () => {
    ...
    const getReq = new Request(
      "http://localhost/api/history?name=packaged-history-proc&event=deploy&format=json",
    );
