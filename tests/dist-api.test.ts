...
describe("dist/api.js compatibility", () => {
  ...
  test("matches named export surface from src/api.ts", async () => {
    const srcKeys = await runModuleProbe(
      "./src/api.ts",
      "Object.keys(mod).sort()",
    );
    const distKeys = await runModuleProbe(
      "./dist/api.js",
      "Object.keys(mod).sort()",
    );
  ...
  test("preserves core utility behavior", async () => {
    const srcValue = await runModuleProbe(
      "./src/api.ts",
      '({ env: mod.parseEnvString("PORT=3000,DEBUG=true"), runtime: mod.calculateRuntime("2026-03-29T00:00:00.000Z"), isWindows: mod.isWindows() })',
    );
    const distValue = await runModuleProbe(
      "./dist/api.js",
      '({ env: mod.parseEnvString("PORT=3000,DEBUG=true"), runtime: mod.calculateRuntime("2026-03-29T00:00:00.000Z"), isWindows: mod.isWindows() })',
    );
  ...
  test("exposes the same database path metadata shape", async () => {
    const srcValue = await runModuleProbe(
      "./src/api.ts",
      "({ dbPath: mod.dbPath, bgrHome: mod.bgrHome })",
    );
    const distValue = await runModuleProbe(
      "./dist/api.js",
      "({ dbPath: mod.dbPath, bgrHome: mod.bgrHome })",
