...
beforeAll(async () => {
  ...
  historyRoute = await import(
    `../dashboard/app/api/history/route?case=${Date.now()}-${Math.random()}`
  );
  logsRoute = await import(
    `../dashboard/app/api/logs/[name]/route?case=${Date.now()}-${Math.random()}`
  );
});
...
describe("export API routes", () => {
  test("history route exports filtered CSV", async () => {
    cleanupHistoryEntries(historyProcess);
    dbMod.addHistoryEntry(historyProcess, "deploy", 1234, {
      status: "ok",
      branch: "main",
    });
    dbMod.addHistoryEntry(historyProcess, "restart", 1234, {
      status: "ignored",
      branch: "dev",
    });

    const req = new Request(
      `http://localhost/api/history?name=${encodeURIComponent(historyProcess)}&event=deploy&metadata=main&format=csv&download=1`,
    );
  ...
  test("logs route exports plain text", async () => {
    ...
    const req = new Request(
      `http://localhost/api/logs/${encodeURIComponent(logProcess)}?tab=stdout&format=text&download=1`,
    );
    ...
    expect(res.headers.get("content-disposition")).toContain(
      `${encodeURIComponent(logProcess)}-stdout.log`,
    );
  ...
  test("logs route exports CSV with line numbers", async () => {
    const req = new Request(
      `http://localhost/api/logs/${encodeURIComponent(logProcess)}?tab=stdout&format=csv`,
    );
    const res = await logsRoute.GET(req, { params: { name: logProcess } });
    const text = await res.text();
  ...
  test("logs route exports JSON metadata payload", async () => {
    const req = new Request(
      `http://localhost/api/logs/${encodeURIComponent(logProcess)}?tab=stderr&format=json&download=1`,
    );
    const res = await logsRoute.GET(req, { params: { name: logProcess } });
    ...
    expect(res.headers.get("content-disposition")).toContain(
      `${encodeURIComponent(logProcess)}-stderr.json`,
    );
