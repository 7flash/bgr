...
if (!g.__bgrResourceHistory) {
  ...
  g.__bgrResourceHistory = new Map<
    string,
    { memory: number[]; cpu: number[]; lastCpuTime: number; lastCheck: number }
  >();
}
...
async function fetchProcesses(): Promise<any[]> {
  return (
    (await api.measure("Fetch processes", async (m) => {
      const procs = getAllProcesses().filter(
        (p: any) => !isInternalProcessName(p.name),
      );
      ...
      const deadPids = new Set(
        pids.filter((pid: number) => !runningPids?.has(pid)),
      );

      if (deadPids.size > 0) {
        const reconciled = await m("Reconcile dead PIDs", () =>
          withTimeout(
            reconcileProcessPids(procs, deadPids),
        ...
        if (reconciled && reconciled.size > 0) {
          ...
          const [newPorts, newResources] = await Promise.all([
            ...
            withTimeout(
              getProcessBatchResources(newPids),
              new Map<number, { memory: number; cpu: number }>(),
            ),
          ]);
      ...
      return procs.map((p: any) => {
        ...
        const ports = running ? portMap?.get(p.pid) || [] : [];
        const res = running
          ? resourceMap?.get(p.pid) || { memory: 0, cpu: 0 }
          : { memory: 0, cpu: 0 };
