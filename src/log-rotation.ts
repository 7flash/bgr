...
export function startLogRotation(
  ...
  console.log(
    `[logs] Log rotation active: max ${formatBytes(maxBytes)}/file, keep ${keepLines} lines, check every ${intervalMs / 1000}s`,
  );
  ...
  return setInterval(() => {
    ...
    if (rotated.length > 0) {
      console.log(
        `[logs] Rotated ${rotated.length} log(s): ${rotated.join(", ")}`,
      );
    }
  }, intervalMs);
}
