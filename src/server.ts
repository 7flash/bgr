...
async function cleanupPort(port: number): Promise<number> {
  ...
  try {
    ...
    console.warn(
      `[server] Port ${port} still busy after first cleanup; retrying`,
    );
    await killProcessOnPort(port);
    ...
    console.warn(
      `[server] ⚠ Could not reclaim port ${port}; falling back to port ${fallback}`,
    );
    return fallback;
  } catch {
...
export async function startServer() {
  ...
  const hasExplicitPort =
    explicitPort !== null && !isNaN(explicitPort) && explicitPort > 0;
  ...
  const resolvedPort = hasExplicitPort
    ? await cleanupPort(requestedPort)
    : requestedPort;
...
function startStickyPortChecker() {
  ...
  console.log(
    `[server] Starting sticky port checker (original: ${_originalPort}, current: ${_currentPort})`,
  );
  ...
  setInterval(async () => {
    ...
    try {
      ...
      if (!pid) {
        console.log(
          `[server] ✓ Original port ${_originalPort} is now available! Consider restarting to reclaim it.`,
        );
