...
export async function handleWatch(
  ...
  const dumpLogsIfDead = async (proc: Process, reason: string) => {
    ...
    console.log(
      chalk.yellow(
        `💀 Process '${options.name}' died immediately after ${reason}—dumping logs:`,
      ),
    );
  ...
  const waitForLogReady = (
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        reject(
          new Error(
            `Log file ${logPath} did not become ready within ${timeoutMs}ms`,
          ),
        );
      }, timeoutMs);
    });
  ...
  const startTails = async (): Promise<(() => void)[]> => {
    ...
    if (logOptions.logType === "both" || logOptions.logType === "stdout") {
      ...
      try {
        ...
        console.warn(
          chalk.yellow(
            `⚠️  Stdout log not ready yet for ${options.name}—starting tail anyway: ${err.message}`,
          ),
        );
      }
      const stop = tailFile(
        currentProcess.stdout_path,
        "",
        chalk.white,
        logOptions.lines,
      );
    ...
    if (logOptions.logType === "both" || logOptions.logType === "stderr") {
      ...
      try {
        ...
        console.warn(
          chalk.yellow(
            `⚠️  Stderr log not ready yet for ${options.name}—starting tail anyway: ${err.message}`,
          ),
        );
      }
      const stop = tailFile(
        currentProcess.stderr_path,
        "",
        chalk.red,
        logOptions.lines,
      );
  ...
  const restartProcess = async (path?: string) => {
    ...
    isRestarting = true;
    const restartReason = path
      ? `restart (change in ${path})`
      : "initial start";
    ...
    console.clear();
    announce(
      `🔄 Restarting process '${options.name}'... [${restartReason}]`,
      "Watch Mode",
    );
    ...
    try {
      ...
      if (died) {
        if (lastRestartPath) {
          console.log(
            chalk.yellow(
              `⚠️  Compile error on change—pausing restarts until manual fix.`,
            ),
          );
          return; // Avoid loop on bad code
        } else {
          error(
            `Failed to start process '${options.name}'. Aborting watch mode.`,
          );
          return;
        }
      }
  ...
  console.clear();
  announce(
    `🚀 Starting initial process '${options.name}' in watch mode...`,
    "Watch Mode",
  );
  ...
  if (!currentProcess) {
    error(
      `Could not start or find process '${options.name}'. Aborting watch mode.`,
    );
    return;
  }
  ...
  const cleanup = async () => {
    ...
    if (procToKill) {
      ...
      if (isRunning) {
        ...
        console.log(
          `process ${procToKill.name} (PID: ${procToKill.pid}) still running`,
        );
      }
    }
    process.exit(0);
