...
export function formatDeployToolError(
...
async function isGitRepo(dir: string): Promise<boolean> {
  return (
    (await pathExists(`${dir}/.git`)) || (await pathExists(`${dir}/.git/HEAD`))
  );
}
...
export async function detectPackageManager(
  ...
  if (
    (await pathExists(`${dir}/bun.lock`)) ||
    (await pathExists(`${dir}/bun.lockb`))
  )
    return "bun";
  if (await pathExists(`${dir}/pnpm-lock.yaml`)) return "pnpm";
  if (await pathExists(`${dir}/yarn.lock`)) return "yarn";
  if (
    (await pathExists(`${dir}/package-lock.json`)) ||
    (await pathExists(`${dir}/npm-shrinkwrap.json`))
  )
    return "npm";
...
export async function deployProcess(name: string): Promise<DeployResult> {
  ...
  if (isInternalProcess(proc.name)) {
    return {
      ...
      ok: false,
      skipped: true,
      reason: "Internal bgrun process skipped",
    };
  }
  ...
  if (!(await isGitRepo(dir))) {
    return {
      ...
      ok: false,
      skipped: true,
      reason: `'${dir}' is not a git repository`,
    };
  }
...
export async function deployAllProcesses(
  group?: string,
